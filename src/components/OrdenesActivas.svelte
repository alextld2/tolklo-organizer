<script lang="ts">
  ...
  // 3. 🔥 FILTRADO REACTIVO: Se ejecuta automáticamente cada vez que cambia el valor de $busquedaGlobal
  $: trabajosFiltrados = trabajosProximos.filter(trabajo => {
    ...
    const descMatches = trabajo.descripcionGeneral ? normalizarTexto(trabajo.descripcionGeneral).includes(query) : false;
...
<div class="space-y-4 w-full font-sans text-[#1A1D21] dark:text-[#EDF0F3]">
  ...
  <div class="flex justify-between items-center">
    <h2 class="text-xl font-semibold tracking-tight text-[#1A1D21] dark:text-[#EDF0F3]">
      Pedidos Activos
    </h2>
    ...
    >
      <span class="material-symbols-rounded text-sm" style="font-variation-settings: 'wght' 300;">
        calendar_today
      </span>
  ...
  <div class="space-y-3 min-h-[360px] flex flex-col justify-between">
    <div class="space-y-3 flex-1">
      {#each trabajosVisibles as trabajo (trabajo.numParte)}
        ...
        <div class="bg-white dark:bg-[#16191D] rounded-2xl p-4 border border-[#E9EBF0] dark:border-[#232830] flex items-center justify-between shadow-xs hover:shadow-sm transition-all group">
          <div class="flex items-center gap-4 min-w-0">
            
            <div class="w-11 h-11 bg-purple-50 dark:bg-[#5C42FF]/10 text-[#5C42FF] dark:text-[#9A85FF] rounded-xl flex flex-col items-center justify-center font-semibold text-xs border border-purple-100/60 dark:border-[#5C42FF]/20 transition-colors group-hover:bg-purple-100/50 dark:group-hover:bg-[#5C42FF]/20 flex-shrink-0">
              <span class="text-[8px] uppercase tracking-tighter opacity-50 font-medium">Nº</span>
              <span>{trabajo.numParte}</span>
            </div>
            
            <div class="min-w-0">
              <p class="text-sm font-semibold text-[#1A1D21] dark:text-[#EDF0F3] uppercase tracking-tight truncate">
                {trabajo.cliente}
              </p>
              <p class="text-xs text-gray-400 dark:text-gray-500 font-medium mt-0.5 truncate">
          ...
          <div class="flex items-center gap-2.5 flex-shrink-0">
            <span class="px-2.5 py-1 text-[9px] font-semibold rounded-lg uppercase tracking-wider border transition-colors
            ...
            <span class="px-2.5 py-1 bg-gray-50 dark:bg-[#1E2228] border border-gray-100 dark:border-gray-800 text-[9px] font-semibold text-gray-400 dark:text-gray-500 rounded-lg uppercase tracking-wider transition-colors">
              {trabajo.area}
            </span>
          </div>
        </div>
      ...
      {:else}
        <div class="bg-white dark:bg-[#16191D] border border-dashed border-gray-200 dark:border-gray-800 rounded-2xl p-12 text-center text-gray-400 dark:text-gray-500 my-auto shadow-xs transition-colors">
          <span class="material-symbols-rounded text-4xl block mb-2 text-gray-300 dark:text-gray-700" style="font-variation-settings: 'wght' 100;">
            search_off
          </span>
          <p class="text-xs font-semibold">No se encontraron partes activos que coincidan con la búsqueda.</p>
        </div>
      {/each}
    </div>
