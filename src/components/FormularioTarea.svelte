<script lang="ts">
  ...
  async function enviarFormulario() {
    if (!numParte || !cliente || !fechaSalida) {
      mostrarToast(
        "Campos incompletos",
        "Por favor, rellena los campos obligatorios marcados con asterisco (*).",
        "error",
      );
      return;
    }
    ...
    try {
      const respuesta = await fetch("/api/nueva-tarea", {
        ...
        body: JSON.stringify({
          ...
          area,
          subcontrata,
          desgloses: desgloses.filter(
            (d) => d.descripcionProducto !== "" && d.cantidad !== null,
          ),
        }),
      });
      ...
      if (respuesta.ok) {
        mostrarToast(
          ...
          "La orden de trabajo ha sido registrada correctamente en el sistema.",
          "success",
        );
        numParte = null;
        cliente = "";
        ...
        mostrarToast(
          ...
          datosAPI.message || "No se pudo guardar la tarea.",
          "error",
        );
      }
    } catch (e) {
...
<div
  class="w-full font-sans flex flex-col h-full text-[#1A1D21] dark:text-[#EDF0F3]"
  on:click={cerrarDesplegables}
>
  {#if toast.visible}
    <div
    ...
    >
      ...
      <div class="flex-1 min-w-0">
        ...
        <p
          class="text-xs font-medium mt-1 leading-normal
            ...
            : 'text-red-800/80 dark:text-red-400/80'}"
        >
          {toast.mensaje}
        </p>
      </div>
      ...
      >
        <span
          ...
          style="font-variation-settings: 'wght' 300;">close</span
        >
      </button>
    </div>
  {/if}
  ...
  <div class="mb-6 flex-shrink-0">
    ...
    <p class="text-xs font-medium text-gray-400 dark:text-gray-500 mt-1">
      Define los detalles del nuevo requerimiento de producción.
    </p>
  </div>

  <form
    on:submit|preventDefault={enviarFormulario}
    class="bg-white dark:bg-[#16191D] rounded-3xl p-8 border border-[#E9EBF0] dark:border-[#232830] shadow-xl shadow-gray-100/10 dark:shadow-none space-y-6 flex-1 overflow-y-auto transition-colors"
  >
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <label
          class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 block"
    ...
    <div class="border-t border-gray-50 dark:border-[#232830] pt-5">
      <div class="flex justify-between items-center mb-3">
        <label
          ...
          >Descripción de Líneas / Subtareas (Desglose 1:N)</label
        >
        <button
          type="button"
        ...
        >
          <span
            ...
            style="font-variation-settings: 'wght' 300;">add</span
          >
      ...
      <div class="space-y-3">
        {#each desgloses as fila, index}
          <div
            class="flex items-center gap-4 bg-gray-50/40 dark:bg-[#1E2228]/20 p-3 rounded-2xl border border-gray-100/80 dark:border-[#232830]"
          >
            <input
              type="text"
            ...
            {#if desgloses.length > 1}
              ...
              >
                <span
                  class="material-symbols-rounded text-lg"
                  style="font-variation-settings: 'wght' 300;">delete</span
                >
              </button>
            {/if}
          </div>
    ...
    <div
      class="grid grid-cols-1 md:grid-cols-3 gap-6 border-t border-gray-50 dark:border-[#232830] pt-5"
    >
      <div class="relative">
        <label
          class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 block"
        ...
        <button
          ...
          on:click|stopPropagation={() => {
            cerrarDesplegables();
            menuComercialAbierto = !menuComercialAbierto;
          }}
        ...
        >
          <div class="flex items-center gap-2">
            <span
              class="material-symbols-rounded text-lg text-purple-500"
              style="font-variation-settings: 'wght' 300;">assignment_ind</span
            >
            <span>{comercial || "Seleccionar comercial..."}</span>
          </div>
          <span
            class="material-symbols-rounded text-sm text-gray-400 transition-transform duration-200 {menuComercialAbierto
              ? 'rotate-180'
        ...
        {#if menuComercialAbierto}
          <div
            class="absolute z-30 w-full bg-white dark:bg-[#16191D] mt-1.5 rounded-2xl shadow-2xl border border-gray-100 dark:border-[#232830] p-1.5 space-y-0.5 animate-scale-up"
          >
            <button
              type="button"
              on:click={() => {
              ...
              class="w-full text-left px-3 py-2.5 rounded-xl text-xs font-medium text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-[#1E2228] transition-colors"
              >Ninguno</button
            >
            {#each listaComerciales as c}
              <button
                ...
                {comercial === c
                  ? 'bg-[#5C42FF]/5 text-[#5C42FF] dark:text-[#9A85FF] font-semibold'
                  : 'text-[#1A1D21] dark:text-[#EDF0F3] hover:bg-gray-50 dark:hover:bg-[#1E2228]'}"
              >
                <span>{c}</span>
                {#if comercial === c}<span
                    class="material-symbols-rounded text-sm">check</span
                  >{/if}
              </button>
            {/each}
          </div>
      ...
      <div class="relative">
        ...
        >
          <div class="flex items-center gap-2">
            <span
              class="material-symbols-rounded text-lg {estado === 'Pendiente'
                ? 'text-amber-500'
                : estado === 'En proceso'
                  ? 'text-blue-500'
                  : 'text-emerald-500'}"
          ...
          <span
            class="material-symbols-rounded text-sm text-gray-400 transition-transform duration-200 {menuEstadoAbierto
              ? 'rotate-180'
              : ''}">keyboard_arrow_down</span
          >
        </button>
          ...
          >
            {#each listaEstados as est}
              ...
              >
                <div class="flex items-center gap-2">
                  <span
                    class="material-symbols-rounded text-base {est ===
                    'Pendiente'
                      ? 'text-amber-500'
                      : est === 'En proceso'
                        ? 'text-blue-500'
    ...
    >
      <div class="relative">
        <label
          class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 block"
          >Diseñador Asignado</label
        >
        <button
          type="button"
          on:click|stopPropagation={() => {
            cerrarDesplegables();
            menuDiseñadorAbierto = !menuDiseñadorAbierto;
          }}
          class="w-full bg-[#F1F3F6] dark:bg-[#1E2228] px-4 py-3.5 rounded-xl text-xs font-medium text-[#1A1D21] dark:text-[#EDF0F3] text-left flex justify-between items-center border border-transparent hover:bg-gray-100/70 dark:hover:bg-gray-800/50 transition-colors cursor-pointer"
        >
          <div class="flex items-center gap-2">
            <span
              class="material-symbols-rounded text-lg text-emerald-500 dark:text-emerald-400"
              style="font-variation-settings: 'wght' 300;">palette</span
            >
            <span>{diseñador || "Seleccionar diseñador..."}</span>
          </div>
          <span
            class="material-symbols-rounded text-sm text-gray-400 transition-transform duration-200 {menuDiseñadorAbierto
              ? 'rotate-180'
              : ''}">keyboard_arrow_down</span
          >
        </button>
        ...
        {#if menuDiseñadorAbierto}
          <div
            class="absolute z-20 w-full bg-white dark:bg-[#16191D] mt-1.5 rounded-2xl shadow-2xl border border-gray-100 dark:border-[#232830] p-1.5 space-y-0.5 animate-scale-up"
          >
            <button
              type="button"
              on:click={() => {
      ...
      <div class="relative">
        <label
          class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 block"
          >Área de Producción</label
        >
        <button
          type="button"
          on:click|stopPropagation={() => {
            cerrarDesplegables();
            menuAreaAbierto = !menuAreaAbierto;
          }}
          class="w-full bg-[#F1F3F6] dark:bg-[#1E2228] px-4 py-3.5 rounded-xl text-xs font-medium text-[#1A1D21] dark:text-[#EDF0F3] text-left flex justify-between items-center border border-transparent hover:bg-gray-100/70 dark:hover:bg-gray-800/50 transition-colors cursor-pointer"
        >
          <div class="flex items-center gap-2">
            <span
              class="material-symbols-rounded text-lg text-blue-500"
              style="font-variation-settings: 'wght' 300;">layers</span
            >
            <span>{area || "Seleccionar área..."}</span>
          </div>
          <span
            class="material-symbols-rounded text-sm text-gray-400 transition-transform duration-200 {menuAreaAbierto
              ? 'rotate-180'
              : ''}">keyboard_arrow_down</span
          >
        </button>
          ...
          >
            {#each listaAreas as a}
              <button
                ...
                class="w-full text-left px-3 py-2.5 rounded-xl text-xs font-medium transition-colors flex justify-between items-center cursor-pointer
                {area === a
                  ? 'bg-[#5C42FF] text-white shadow-xs font-semibold'
                  : 'text-[#1A1D21] dark:text-[#EDF0F3] hover:bg-gray-50 dark:hover:bg-[#1E2228]'}"
              >
                <span>{a}</span>
                {#if area === a}<span class="material-symbols-rounded text-sm"
                    >check</span
                  >{/if}
              </button>
            {/each}
      ...
      <div>
        <label
          class="text-[10px] font-semibold text-orange-500 uppercase tracking-widest mb-2 flex items-center gap-1"
        >
          <span
            class="material-symbols-rounded text-sm text-orange-500"
    ...
    <div
      class="border-t border-gray-50 dark:border-[#232830] pt-6 flex flex-col sm:flex-row justify-between items-center gap-4 flex-shrink-0"
    >
      <p
        class="text-[11px] font-medium text-gray-400 dark:text-gray-500 text-center sm:text-left"
      >
        Los campos con asterisco (*) son de obligado cumplimiento para almacenar
        el parte.
      </p>
      ...
      <div class="flex items-center gap-4 w-full sm:w-auto">
        <a
          href="/"
          class="flex-1 sm:flex-none text-center px-8 py-3.5 border border-dashed border-[#5C42FF]/40 dark:border-gray-700 text-[#5C42FF] dark:text-gray-400 font-medium text-xs rounded-full hover:bg-purple-50 dark:hover:bg-gray-800 transition-colors cursor-pointer"
          >Cancelar</a
        >
        <button
          type="submit"
...
<style>
  ...
  .animate-slide-up-center {
    animation: slide-up-center 0.28s cubic-bezier(0.16, 1, 0.3, 1) forwards;
  }
</style>
